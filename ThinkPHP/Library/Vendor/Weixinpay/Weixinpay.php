...
class Weixinpay {
    ...
    private $config=array(
        'APPID'              => 'wx76b04f25405b5dbe', // 微信支付APPID
        'MCHID'              => '1498275822', // 微信支付MCHID 商户收款账号
        'KEY'                => 'TwD9sjEHKaAKaVjoMRExmCRno9cuiR17', // 微信支付KEY
        'APPSECRET'          => '',  //公众帐号secert
        'NOTIFY_URL'         => 'http://p.wyuek.com/index.php/Home/Wxnotify/index', // 回调地址
        );
     ...
     * @param  array $order 订单 必须包含支付所需要的参数 body(产品描述)、total_fee(订单金额)、out_trade_no(订单号)、product_id(产品id)、trade_type(类型：JSAPI，NATIVE，APP)
     */
    public function unifiedOrder_h5($order){
        // 获取配置项
        $weixinpay_config=$this->config;
        $config=array(
            'appid'=>$weixinpay_config['APPID'],
     ...
     * @param  array $order 订单 必须包含支付所需要的参数 body(产品描述)、total_fee(订单金额)、out_trade_no(订单号)、product_id(产品id)、trade_type(类型：JSAPI，NATIVE，APP)
     */
    public function unifiedOrder_app($order){
        // 获取配置项
        $weixinpay_config=$this->config;
        $config=array(
            ...
            'notify_url'=>'http://p.wyuek.com/index.php/Home/Wxnotify/app'
            );
        // 合并配置数据和订单数据
        $data=array_merge($order,$config);
        // 生成签名
        $sign=$this->makeSign_app($data);
        $data['sign']=$sign;
        $xml=$this->toXml($data);
        $url = 'https://api.mch.weixin.qq.com/pay/unifiedorder';//接收xml数据的文件
        $header[] = "Content-type: text/xml";//定义content-type为xml,注意是数组
    ...
    public function notify(){
        ...
        file_put_contents('/tmp/appnotify.txt',var_export($xml,true)."\n",FILE_APPEND);
        // 转成php数组
    ...
    **/
     ...
     * @return 签名，本函数不覆盖sign成员变量，如要设置签名需要调用SetSign方法赋值
     */
    public function makeSign_app($data){
        // 去空
        $data=array_filter($data);
        //签名步骤一：按字典序排序参数
        ksort($data);
        ...
        //签名步骤二：在string后加入KEY
        $config=$this->config;
        $string_sign_temp=$string_a."&key=mO1IJTcrXourVguxPpb07W3n40OkWgQX";
        //签名步骤三：MD5加密
        $sign = md5($string_sign_temp);
        // 签名步骤四：所有字符转为大写
        $result=strtoupper($sign);
